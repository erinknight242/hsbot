...
module.exports = (robot) ->
  ...
  robot.respond /monopoly start new game$/i, (msg) ->
    robot.brain.set 'monopolyBoard', [
      ...
      { name: "Austin Railroad", cost: 200, rent: 25, mortgage: 100, owner: null, mortgaged: false }
  ...
  robot.respond /monopoly roll$/i, (msg) ->
    ...
    if data
      ...
      msg.send players[playerIndex].name + ' rolls ' + currentRoll.total + ', advances to ' + data[player.location].name + '.'
      ...
      if _.contains([2, 17, 33], player.location)
        ...
        if !data[player.location].owner
          ...
          if (data[player.location].owner == player.name)
            ...
            msg.send 'Pay ' + data[player.location].owner + ' $' + owes + bankerInstuctions
          setNextPlayer()
